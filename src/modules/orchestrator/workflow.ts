...
import { taskStatusForRun } from "../../shared/domain/status.ts";
import type {
  AgentRun,
  Change,
  ChangeComment,
...
import * as git from "../../shared/git.ts";
...
export class Workflow {
  ...
  /** ponytail: in-memory, one chain per task — a queue on disk if a second orchestrator appears. */
  private readonly taskStatusQueue = new Map<string, Promise<void>>();
  ...
  private syncTaskStatus(run: AgentRun, status: RunStatus) {
    ...
    policy.enqueue(
      ...
      run.taskId,
      () => this.taskProvider(run.taskId).updateStatus(run.taskId, taskStatus),
      (err) => this.log(`task ${run.taskId}: ${briefError(err)}`),
    );
  }
  ...
  async start(project: Project, task: Task): Promise<AgentRun> {
    ...
    try {
      ...
      // a resumed change is already implemented; planning it again would redo finished work
      const plan = policy.resolveAgentRole("plan", this.deps.config, cfg);
      const planning = !feedback && Boolean(plan.prompt.trim() || plan.skill.trim());
      this.prompted.set(run.id, { at: Date.now(), sawWorking: false });
      await this.deps.herdr.prompt(
        run.herdrAgentId,
        planning
          ? policy.buildPlanPrompt(task, { branch, prefix: plan.prompt, skill: plan.skill })
          : [
      ...
      this.transition(run, planning ? "planning" : "working");
      return run;
    } catch (err: any) {
   ...
   *
   * The name is asked of herdr directly: `agent list` does not always report it, and a start
   * over an agent we failed to recognise fails twice over — the name is taken and the pane it
   * sits in is no longer a shell.
   */
  private async spawnAgent(
    ...
    const ours =
      live.find((a) => a.name === run.herdrAgentId) ??
      ((await this.deps.herdr.getAgentStatus(run.herdrAgentId)) !== "unknown"
        ? { name: run.herdrAgentId, paneId: ws.paneId }
        : undefined);
    if (ours) {
    ...
    try {
      ...
      // the agent behind the name is gone; only its registration is left in the way
      if (!policy.isStaleAgentName(String(err?.message ?? err))) throw err;
      await this.deps.herdr.clearAgentName(run.herdrAgentId);
      this.event("AgentNameCleared", run, { agent: run.herdrAgentId });
      await start();
    }
  }
  ...
  private async observeAgent(run: AgentRun): Promise<void> {
    ...
    if (status === "working") {
      ...
      if (run.status !== "working" && run.status !== "planning") this.transition(run, "working");
      return;
    }
    ...
    if (ticks >= 2) {
      this.idleTicks.delete(run.id);
      if (run.status === "planning") await this.startWork(run);
      else this.transition(run, "validating");
    }
  }

  /** The planning pass is over: the same agent, now holding the plan, is asked to build it. */
  private async startWork(run: AgentRun): Promise<void> {
    const task = db.getTask(this.deps.db, run.taskId);
    if (!task) {
      this.fail(run, `task ${run.taskId} is gone`);
      return;
    }
    const cfg = this.projectConfigById(run.projectId);
    const dev = policy.resolveAgentRole("dev", this.deps.config, cfg);
    this.event("PlanPosted", run);
    this.prompted.set(run.id, { at: Date.now(), sawWorking: false });
    await this.deps.herdr.prompt(
      run.herdrAgentId,
      policy.buildPrompt(task, {
