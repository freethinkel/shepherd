...
import { normalizeAgentStatus } from "../../shared/domain/status.ts";
import { briefError } from "../../shared/log.ts";
import type { AgentStatus } from "../../shared/domain/types.ts";
...
export class HerdrClient {
  ...
  /** Frees a name still registered to an agent that is gone, so `agent start` can take it again. */
  async clearAgentName(agent: string): Promise<void> {
    await this.call(["agent", "rename", agent, "--clear"]);
  }
