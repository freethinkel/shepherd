...
// a miss is null here, not undefined — `!== undefined` would report every event as present
export const hasEvent = (db: Db, runId: string, type: string): boolean =>
  db.prepare(`SELECT 1 FROM events WHERE run_id = ? AND type = ? LIMIT 1`).get(runId, type) != null;
...
export const lastEventAt = (db: Db, taskId: string, type: string): Date | undefined => {
  const row = db
    ...
    .get(taskId, type) as { at: string } | null;
